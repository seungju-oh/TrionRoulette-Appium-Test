...
- **AI를 튜터(Tutor)로 활용한 집요한 역질문 학습:** 완성된 코드를 맹목적으로 복사하기 전, AI에게 "이 XPath 쿼리는 어떤 원리로 두 OS를 모두 잡는가?", "왜 여기서 1.5초의 동적 대기가 필요한가?" 등 **코드 한 줄 한 줄의 작동 원리와 배경 지식을 집요하게 되묻고 토론**하며 자동화 프레임워크의 근본적인 동작 방식을 파고들었습니다.
- **완벽한 내재화와 주석(Annotation) 작성:** AI와의 문답을 통해 완벽히 이해한 Appium의 `WebDriverWait` 메커니즘, 모바일 UI 렌더링 생명주기, 크로스플랫폼 우회 전략 등을 바탕으로, 코드 전체에 **저만의 언어로 상세한 주석을 작성**했습니다.
- **결과:** 이 과정을 통해 AI의 산출물을 단순 '차용'하는 수준을 넘어, 자동화 테스트 코드 100%를 나의 것으로 '내재화(Internalize)'함으로써 코드에 대한 완벽한 통제권과 오너십을 확보했습니다.
...
| **확률 (Mode C)** | - 소수점 둘째 자리(n.nn%) 초과 입력 시 정규식 마스킹 방어 검증<br>- 0% 확률 입력 시 동적 렌더링 및 당첨 풀(Pool) 제외 확인<br>- 확률 총합 부동소수점 오차(99.99%, 100.01%) 시 스핀 방어 검증 |
